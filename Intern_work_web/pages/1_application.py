...
# # Page Config
# st.set_page_config(
#     page_title="TEQNITEHUB Application",
#     page_icon="✨",
#     layout="centered",
...
#             <h1 style='color: #fdbb2d; margin-bottom: 10px;margin-top: -80px;'>TEQNITEHUB Academy</h1>
...
#         phone = col2.text_input("Phone Number", placeholder="+92 (123) 456-7890")
...
#                         </ol>
#                         <p style='font-style: italic;'>We appreciate your interest in TEQNITEHUB Academy!</p>
#                     </div>
#                     """, unsafe_allow_html=True)
...
def firebase_store_application(user_data):
    ...
    firestore_data = {
        "fields": {
            ...
            "major": {"stringValue": user_data["major"]},
            "program": {"stringValue": user_data["program"]},
            "internship_duration": {"stringValue": user_data["internship_duration"]},
            "graduation_year": {"integerValue": str(user_data["graduation_year"])},
            "programming_languages": {
                "arrayValue": {
                    "values": [{"stringValue": lang} for lang in user_data["programming_languages"]]
...
def show_application_form():
    ...
    with st.form("application_form", clear_on_submit=True):
        ...
        # Add Internship Duration field
        internship_duration = st.selectbox("Internship Duration*",
        ...
        if submitted:
            ...
            if not major: errors.append("Major is required")
            if not program: errors.append("Program of interest is required")
            if not internship_duration: errors.append("Internship duration is required")
            if not project_description: errors.append("Project description is required")
            if not password: errors.append("Password is required")
            if password != confirm_password: errors.append("Passwords do not match")
            if not validate_password(password): errors.append("Password must be at least 8 characters")
